...
```bash
sam_lambda_59$ sam logs -n HelloEfsFunction --stack-name "sam_lambda_59" --tail
```
...
```bash
sam_lambda_59$ pip install -r tests/requirements.txt --user
# unit test
sam_lambda_59$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sam_lambda_59$ AWS_SAM_STACK_NAME="sam_lambda_59" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "sam_lambda_59"
```
